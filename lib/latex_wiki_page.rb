...
module ModuleLatexWikiPage
  ...
  def to_latex()
    ...
    ### BUG: Exception if NULL label => requires some default label
    ### TODO: **************
    ...
    page_txt.gsub!(/(\s|^)\[\[(.*?)(\|(.*?)|)\]\]/i) do |_|
      ref = $2
      label = $4
      label = "(missing label)" unless defined? label
      logger.debug " check ref for #{ref} -> #{label}"
    ...
    if self.latex_add_chapter
      # Add chapter tag
      page_txt = "\n\\chapter{#{self.chapter_name.gsub('_', ' ')}} \\label{page:#{self.wiki_page.title.gsub(' ', '_')}}\n" + page_txt
    else
